    ...
    for id in task_ids {
        ...
        if let Some(value) = store.get(&task_key) {
            match serde_json::from_value::<Task>(value) {
                ...
                Err(e) => eprintln!(
                    "Warning: Failed to deserialize task with key {}: {}",
                    task_key, e
                ),
            }
        } else {
...
pub async fn sync_tasks(
    ...
    let updated_task_ids: Vec<String> = sorted_updated_tasks
        .iter()
    ...
    for task in updated_tasks {
        ...
        if let Some(outdated_task) = outdated_tasks.get(&task.id) {
            ...
            // if task does not exist yet; add
            store.set(&task_key, updated_task);
    ...
    let updated_task_ids_set: HashSet<String> =
        updated_task_ids.iter().map(|id| id.clone()).collect();
    ...
    let updated_task_ids_serde: Value = Value::Array(
        updated_task_ids
            .iter()
